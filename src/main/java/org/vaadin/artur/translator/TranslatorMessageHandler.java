...
@JsModule("./dev-tools-translator.ts")
public class TranslatorMessageHandler implements DevToolsMessageHandler {
    ...
    public TranslatorMessageHandler() {
    }
    ...
    private void translateUI(JsonObject data,
        ...
        session.accessSynchronously(() -> {
            ...
            for (HasElement viewOrLayout : activeRoutes) {
                ...
                File javaFile = location.findJavaFile(getConfiguration());
                try {
                    CompilationUnit cu = Util
                            .parseSource(Util.readFile(javaFile));
                    if (!Util.hasImport(cu, translatorImport)) {
    ...
    private void copyTranslator(String mainPackage) throws IOException {
        File targetFolder = new File(getConfiguration().getJavaSourceFolder(),
        ...
        source = source.replaceFirst("package .*;",
                "package " + mainPackage+";");
    ...
    private void writeTranslationFile(String localeString,
        ...
        File file = new File(getConfiguration().getJavaResourceFolder(),
                filename);
